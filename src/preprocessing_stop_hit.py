...
def balance_dataset(df, target_col="stop_hit", ratio=2.5):
    """
    ...
    print(df["stop_hit"].value_counts(normalize=True).rename("percent").to_frame()*100)

    return balanced_df

def perform_time_based_split(df, target_col="stop_hit", train_size=0.7, valid_size=0.15):
    """
    ...
    for _, chunk in df.groupby("time_bin"):
        ...
        n_chunk = len(chunk)
        n_train = int(n_chunk * train_size)
        n_valid = int(n_chunk * valid_size)
    ...
    for name, d in zip(["Train", "Valid", "Test"], [train, valid, test]):
        counts = d[target_col].value_counts(normalize=True).mul(100).round(2).to_dict()
...
def drop_irrelevant_and_leakage_cols(df, target_col='stop_hit'):
    """
    ...
    if target_col == 'stop_hit' and 'target_hit' in df.columns:
        to_drop.append('target_hit')
    elif target_col == 'target_hit' and 'stop_hit' in df.columns:
...
def main():
    ...
    try:
        ...
        df = drop_irrelevant_and_leakage_cols(df, target_col='stop_hit')
        ...
        # 6: Balance TRAIN only (correct stage)
        train = balance_dataset(train, target_col="stop_hit", ratio=2.5)

        # Step 7: Clip Outliers (fit on balanced train)
        train, valid, test = clip_outliers(train, valid, test)

        # Step 8: Scale Features (fit on balanced train)
        ...
        train.to_csv(splits_dir / "train_stop_hit.csv", index=False)
        valid.to_csv(splits_dir / "valid_stop_hit.csv", index=False)
        test.to_csv(splits_dir / "test_stop_hit.csv", index=False)
